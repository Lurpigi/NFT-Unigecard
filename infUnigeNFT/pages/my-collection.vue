<template>
    <div class="min-h-screen w-full text-white" :class="{ 'overflow-x-hidden': true }">
      <div class="relative z-0">
        <div class="absolute inset-0 bg-[url('/imgs/bg.png')] bg-cover bg-center bg-no-repeat opacity-20 -z-10"></div>
      </div>
      ...
      <div class="px-6 py-10 max-w-6xl mx-auto">
        <div class="mb-8 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div class="flex items-center gap-2">
            ...
            <Switch v-model:checked="showOnlyOwned" class="data-[state=checked]:bg-yellow-400" />
          </div>
        </div>
          ...
          >
            <CardContent class="w-full h-full flex items-center justify-center p-2 relative">
              <template v-if="hasCard(id)">
                <img :src="`/cards/${id}.png`" :alt="`Carta #${id}`" class="object-contain w-full h-full" />
                <div class="absolute bottom-2 right-2 bg-black/70 text-white text-xs px-2 py-1 rounded">
                  ×{{ userCards[id] }}
                </div>
              </template>
              <template v-else>
                <div class="flex flex-col items-center justify-center text-white/40 text-sm text-center px-4">
                  <span>Carta #{{ id }}</span>
  ...
  function goToCard(id) {
    window.location.href = `https://garza/${id}`
  }
